...
let NEXT_MASS_TIMBER_WALL_ID = 60000; // Allow maximum 10000 mass timber walls only. ID can go from 60000 to 69999.

export class MassTimberWall extends RectangularObject{
	constructor(x, y, length, width, rotation){
		super(x, y, length, width, rotation);
		this.id = NEXT_MASS_TIMBER_WALL_ID++;
		
		this.objects = []; // Doors and windows
		
		this.faces = [];
		
		this.faces.push(new Face(Face.FACE_MASS_TIMBER_SIDE_1));
		this.faces.push(new Face(Face.FACE_MASS_TIMBER_SIDE_2));
		this.faces.push(new Face(Face.FACE_MASS_TIMBER_SIDE_3));
		this.faces.push(new Face(Face.FACE_MASS_TIMBER_SIDE_4));
	}
	
	cloneWithDifferentPositionInSuite(){
		const wall = new MassTimberWall(this.x, this.y, this.length, this.width, this.rotation);
