...
export class NavigationController {
  ...
  initEvents() {
    ...
    $("#nav_previous").addEventListener("click", () => {
      ...
      if (
        self.currentStep != requested_step &&
        (requested_step >= 1 || requested_step <= self.maxAllowedStep)
      ) {
  ...
  async updateUI() {
    ...
    for (let i = 1; i <= this.maxAllowedStep; i++) {
      ...
      if (i > 1) {
        $("[data-navigation-step-connector='" + (i - 1) + "']").classList.add(
          "available"
        );
      }
    }
    if (this.currentStep <= this.maxAllowedStep) {
      $("[data-navigation-step='" + this.currentStep + "']").classList.add(
        "active"
      );
    ...
    $(
      "[data-sidebar-type='step_" + this.currentStep + "_instruction']"
    ).classList.add("shown");

    // Adjust the left sidebar area for buttons
    if (this.currentStep == 2) {
      $(
        "[data-sidebar-button-element-type='perimeter_wall'][data-sidebar-button-action='fire']"
      ).disabled = true;
      $(
        "[data-sidebar-button-element-type='perimeter_wall'][data-sidebar-button-action='objects']"
      ).disabled = true;
    }
    ...
    if (this.currentStep == 4) {
      ...
      for (let i = 0; i < this.suite.perimeterWalls.length; i++) {
        ...
        $("[name='threeD_wall_visibility'][value='" + (i + 1) + "']").checked =
          !this.threeDRenderer.invisibleWallIndices.includes(i);
        indices_that_exist.push(i);
      }

      this.threeDRenderer.invisibleWallIndices =
        this.threeDRenderer.invisibleWallIndices.filter((index) =>
    ...
    // Outcome
    if (this.currentStep == 5) {
      await this.outcomeService.generateOutcome(
        this.suite,
        this.suiteRenderer,
